...
//    Serial.println("MPU6050 Conectado");  
    ...
    // Conversão dos dados do acelerômetro (de 16 bits para g)
    float accelX = ax / 16384.0;
    float accelY = ay / 16384.0;
    float accelZ = az / 16384.0;

    // Cálculo opcional do pitch e roll
    ...
    int roll = (atan(accelY / sqrt(pow(accelX, 2) + pow(accelZ, 2))) * 180 / PI)-
    (int((atan(accelY / sqrt(pow(accelX, 2) + pow(accelZ, 2)))) * 180 / PI)%5)+90;
    int pitch = (atan(-accelX / sqrt(pow(accelY, 2) + pow(accelZ, 2))) * 180 / PI)-
    (int(atan(-accelX / sqrt(pow(accelY, 2) + pow(accelZ, 2))) * 180 / PI)%5)+90;
    int yaw = (atan(sqrt(pow(accelX, 2) + pow(accelY, 2)) / accelZ) * 180 / PI)-
    (int(atan(sqrt(pow(accelX, 2) + pow(accelY, 2)) / accelZ) * 180 / PI)%5);
...
//    Serial.print(" ");
//    Serial.println(yaw);
    matriz[1][acel-1] = roll;
    matriz[2][acel-1] = pitch;
    matriz[3][acel-1] = yaw;
